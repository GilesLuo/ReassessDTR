...
with open(os.path.join(export_dir, 'step_3_start.pkl'), 'rb') as file:
...
for icustayidrow in range(1, sepsis.shape[0] + 1):
    ...
    elif 10000 <= icustayid < 20000:
        temp = ce1020[ce1020[:, 0] == icustayid + 200000, :]
    elif 20000 <= icustayid < 30000:
        temp = ce2030[ce2030[:, 0] == icustayid + 200000, :]
    elif 30000 <= icustayid < 40000:
    ...
    if (t.size != 0):
        ...
        qstime[icustayid - 1, 3] = demog.iloc[
            np.where(demog['icustay_id'] == icustayid + 200000)[0], 4]  # dischargetime
...
dataheaders5 = ['bloc', 'icustayid', 'charttime', 'gender', 'age', 'elixhauser', 're_admission', 'died_in_hosp',
                'died_within_48h_of_out_time', 'mortality_90d', 'delay_end_of_record_and_discharge_or_death', 'SOFA',
                'SIRS', 'Weight_kg', 'GCS', 'HR', 'SysBP', 'MeanBP', 'DiaBP', 'RR', 'SpO2', 'Temp_C', 'FiO2_1',
                'Potassium', 'Sodium', 'Chloride', 'Glucose', 'BUN', 'Creatinine', 'Magnesium', 'Calcium', 'Ionised_Ca',
                'CO2_mEqL', 'SGOT', 'SGPT', 'Total_bili', 'Albumin', 'Hb', 'WBC_count', 'Platelets_count', 'PTT', 'PT',
                'INR', 'Arterial_pH', 'paO2', 'paCO2', 'Arterial_BE', 'HCO3', 'Arterial_lactate', 'mechvent',
                'Shock_Index', 'PaO2_FiO2', 'median_dose_vaso', 'max_dose_vaso', 'input_total', 'input_4hourly',
...
reformat3t.loc[ii, 'elixhauser'] = np.nanmedian(
    reformat3t.loc[:, 'elixhauser'])  # use the median value / only a few missing data points
...
ii = reformat4t['Shock_Index'] >= mquantiles(reformat4t['Shock_Index'], 0.999, alphap=0.5, betap=0.5)[
    0]  # replace outliers with 99.9th percentile
...
MIMICtable.to_csv(os.path.join(export_dir, 'mimictable.csv'), index=False, na_rep='NaN')

# Save pickle 
with open(os.path.join(export_dir, 'step_4_start.pkl'), 'wb') as file:
