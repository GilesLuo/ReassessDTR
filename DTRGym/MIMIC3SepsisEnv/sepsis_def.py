...
export_dir = "./offline_data"
print('Load abx')
abx = pd.read_csv(f'{export_dir}/abx.csv', delimiter='|').values
print('Load culture')
culture = pd.read_csv(f'{export_dir}/culture.csv', delimiter='|').values
print('Load microbio')
microbio = pd.read_csv(f'{export_dir}/microbio.csv', delimiter='|').values
print('Load demog')
demog = pd.read_csv(f'{export_dir}/demog.csv', delimiter='|')  # read as DataFrame
print('Load ce010')
ce010 = pd.read_csv(f'{export_dir}/ce010000.csv', delimiter='|').values
print('Load ce1020')
ce1020 = pd.read_csv(f'{export_dir}/ce1000020000.csv', delimiter='|').values
print('Load ce2030')
ce2030 = pd.read_csv(f'{export_dir}/ce2000030000.csv', delimiter='|').values
print('Load ce3040')
ce3040 = pd.read_csv(f'{export_dir}/ce3000040000.csv', delimiter='|').values
print('Load ce4050')
ce4050 = pd.read_csv(f'{export_dir}/ce4000050000.csv', delimiter='|').values
print('Load ce5060')
ce5060 = pd.read_csv(f'{export_dir}/ce5000060000.csv', delimiter='|').values
print('Load ce6070')
ce6070 = pd.read_csv(f'{export_dir}/ce6000070000.csv', delimiter='|').values
print('Load ce7080')
ce7080 = pd.read_csv(f'{export_dir}/ce7000080000.csv', delimiter='|').values
print('Load ce8090')
ce8090 = pd.read_csv(f'{export_dir}/ce8000090000.csv', delimiter='|').values
print('Load ce90100')
ce90100 = pd.read_csv(f'{export_dir}/ce90000100000.csv', delimiter='|').values
print('Load labU')
labU = np.vstack([pd.read_csv(f'{export_dir}/labs_ce.csv', delimiter='|').values,
                  pd.read_csv(f'{export_dir}/labs_le.csv', delimiter='|').values])
print('Load MV')
MV = pd.read_csv(f'{export_dir}/mechvent.csv', delimiter='|').values
print('Load inputpreadm')
inputpreadm = pd.read_csv(f'{export_dir}/preadm_fluid.csv', delimiter='|').values
print('Load inputMV')
inputMV = pd.read_csv(f'{export_dir}/fluid_mv.csv', delimiter='|').values
print('Load inputCV')
inputCV = pd.read_csv(f'{export_dir}/fluid_cv.csv', delimiter='|').values
print('Load vasoMV')
vasoMV = pd.read_csv(f'{export_dir}/vaso_mv.csv', delimiter='|').values
print('Load vasoCV')
vasoCV = pd.read_csv(f'{export_dir}/vaso_cv.csv', delimiter='|').values
print('Load UOpreadm')
UOpreadm = pd.read_csv(f'{export_dir}/preadm_uo.csv', delimiter='|').values
print('Load UO')
UO = pd.read_csv(f'{export_dir}/uo.csv', delimiter='|').values
...
for icustayid in range(1, 100001):
    ...
    if qst > 0:  # if we have a flag
        ...
        if d1[0] > 6574:  # if older than 18 years old
            ...
            ii = (temp[:, 1] >= qst - (winb4 + 4) * 3600) & (
                        temp[:, 1] <= qst + (winaft + 4) * 3600)  # time period of interest -4h and +4h
            ...
            temp2 = labU[ii, :]
            ii = (temp2[:, 1] >= qst - (winb4 + 4) * 3600) & (
                        temp2[:, 1] <= qst + (winaft + 4) * 3600)  # time period of interest -4h and +4h
            temp2 = temp2[ii, :]  # only time period of interest

            # Mech Vent + ?extubated
            ii = MV[:, 0] == icustayid + 200000
            temp3 = MV[ii, :]
            ii = (temp3[:, 1] >= qst - (winb4 + 4) * 3600) & (
                        temp3[:, 1] <= qst + (winaft + 4) * 3600)  # time period of interest -4h and +4h
            ...
            if (len(temp_list) == 0):
                ...
                t = np.unique(
                    np.vstack(temp_list))  # list of unique timestamps from all 3 sources / sorted in ascending order
            ...
            if (t.size != 0):
                ...
                qstime[
                    icustayid - 1, 0] = qst  # flag for presumed infection / this is time of sepsis if SOFA >=2 for this patient
...
for i in range(npt):
    ...
    demogi = np.where(demog.loc[:, 'icustay_id'] == icustayid + 200000)[0][0]
    dem = np.array([demog.loc[demogi, 'gender'], demog.loc[demogi, 'age'], demog.loc[demogi, 'elixhauser'],
                    demog.loc[demogi, 'adm_order'] > 1, demog.loc[demogi, 'morta_hosp'],
                    np.abs(demog.loc[demogi, 'dod'] - demog.loc[demogi, 'outtime']) < (24 * 3600 * 2),
                    demog.loc[demogi, 'morta_90'], (qstime[icustayid - 1, 3] - qstime[icustayid - 1, 2]) / 3600])
...
# save cohort
sepsis.to_csv(os.path.join(export_dir, 'sepsis_mimiciii.csv'), index=False, na_rep='NaN')

# Save to pickle for step 3 
with open(os.path.join(export_dir, 'step_3_start.pkl'), 'wb') as file:
